...
export default class Lightning {
  constructor() {
    ...
    this.randomLightningRadius = 30 // radius around the camera/player
  ...
  startRandomLightning() {
    setInterval(() => {
      ...
      const radius = this.randomLightningRadius * Math.sqrt(Math.random()) // Uniform distribution
      const x = Math.cos(angle) * radius
      const z = Math.sin(angle) * radius

      const center = this.camera.position.clone() // You could also use a fixed Vector3

      const startPosition = new THREE.Vector3(center.x + x, 50, center.z + z) // Start above the ground
      const endPosition = new THREE.Vector3(center.x + x, 0, center.z + z) // strike on ground level
